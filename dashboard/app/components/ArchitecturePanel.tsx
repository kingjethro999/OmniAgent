...
import { Code, Cpu, Smartphone, Monitor, CheckCircle2, Circle } from 'lucide-react';
  ...
  return (
    <div className="p-4 rounded-lg border border-[var(--border-subtle)] bg-[var(--bg-card)]">
      <div className="flex items-center gap-2 mb-3 pb-2 border-b border-[var(--border-subtle)]">
        ...
        <h2 className="text-sm font-semibold text-[var(--text-primary)]">OmniAgent Engine Architecture & Roadmap</h2>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 text-xs">
        {phases.map((p) => (
          <div key={p.num} className="p-3 rounded-md border border-[var(--border-subtle)] bg-[var(--bg-surface)]">
            <div className="flex items-center justify-between mb-1">
              <span className="font-mono font-semibold text-[var(--accent-cobalt)]">{p.num}</span>
              <CheckCircle2 className="w-3.5 h-3.5 text-emerald-500" />
            </div>
            <h3 className="font-semibold text-[var(--text-primary)] mb-0.5">{p.title}</h3>
            <span className="text-[11px] font-mono text-[var(--text-tertiary)] block mb-1.5">{p.stack}</span>
