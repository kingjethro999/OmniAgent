...
export default function Home() {
  ...
  const handleDispatchTask = async (taskText: string) => {
    ...
    try {
      ...
      const isLocal =
        !taskText.toLowerCase().includes("essay") &&
        !taskText.toLowerCase().includes("matrix") &&
        !taskText.toLowerCase().includes("math");
      ...
      await fetch("/api/events", {
        ...
        body: JSON.stringify({
          ...
          message: isLocal
            ? "Executing on-device via C++ Core (Phi-4-mini)"
            : "Executing via Cloud Adapter (OpenAI / Gemini)",
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="text-xs font-bold text-[var(--text-primary)]">
                  OmniAgent v0.2.1 Native Applications Released
                </span>
                <span className="px-1.5 py-0.2 text-[10px] font-mono font-semibold rounded bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
                  NEW
                </span>
              </div>
              <p className="text-[11px] text-[var(--text-secondary)]">
                Run the private on-device Voice Assistant on Linux, Windows, or
                Android.
              </p>
            </div>
          </div>
